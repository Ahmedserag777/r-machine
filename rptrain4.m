% Resilient propagation for 5-layer networks
%
% model=rptrain4(P,T,h,act,pr,srU,srD,lrU,lrD,d0,maxIt,minSmE)
%
...
%
function model=rptrain4(P,T,h,act,pr,srU,srD,lrU,lrD,d0,maxIt,minSmE)
if nargin<12,
   minSmE=1e-8;
end
if nargin<11,
...
if nargin<9,
   lrD=[1e-24,1e-24,1e-24,1e-24,1e-24];
end   
if nargin<8,
   lrU=[1e24,1e24,1e24,1e24,1e24];
end   
if nargin<7,
   srD=[0.5,0.5,0.5,0.5,0.5];
end
if nargin<6,
   srU=[1.2,1.2,1.2,1.2,1.2];
end
if nargin<5,
   pr=[0,0,0,0,0];
end
...
%calculate the delta sum
    ...
    for ii=1:(h(4)+1),
     for jj=1:m,
      for kk=1:N,
          dW5(ii,jj)=dW5(ii,jj)+dE(kk,jj)*Q5(kk,jj)*L4(kk,ii);
      end
     end
    end        
    for ii=1:(h(4)+1),
