% Resilient propagation for 4-layer networks
%
% model=rptrain3(P,T,h,act,pr,srU,srD,lrU,lrD,d0,maxIt,minSmE)
%
...
%
function model=rptrain3(P,T,h,act,pr,srU,srD,lrU,lrD,d0,maxIt,minSmE)
if nargin<12,
   minSmE=1e-8;
end
if nargin<11,
...
if nargin<9,
   lrD=[1e-24,1e-24,1e-24,1e-24];
end   
if nargin<8,
   lrU=[1e24,1e24,1e24,1e24];
end   
if nargin<7,
   srD=[0.5,0.5,0.5,0.5];
end
...
while (it<=maxIt)&&(smE1>minSmE),
    ...
    for ii=1:(h(3)+1),
     for jj=1:m,
      for kk=1:N,
          dW4(ii,jj)=dW4(ii,jj)+dE(kk,jj)*Q4(kk,jj)*L3(kk,ii);
      end
     end
    end        
    for ii=1:(h(3)+1),
     for jj=1:m,
         ...
         if dWW>0,
            dD4(ii,jj)=1.2*duD4(ii,jj);
         elseif dWW<0,
                dD4(ii,jj)=0.5*duD4(ii,jj);
         elseif dWW==0,
         ...
         elseif dW4(ii,jj)<0,
                model.W4(ii,jj)=model.W4(ii,jj)+dD4(ii,jj);
         elseif dW4(ii,jj)==0,
                model.W4(ii,jj)=model.W4(ii,jj)+0;
         end
     end
    end
